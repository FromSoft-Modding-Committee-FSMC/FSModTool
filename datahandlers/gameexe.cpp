...
GameEXE::GameEXE(KFMTFile& file_) : KFMTDataHandler(file_)
{
    switch (core.currentVersionedGame())
    {
        case KFMTCore::VersionedGame::KF2Jv1_0:
            ...
            for (size_t i = 0; i < 6; i++)
            {
                shopStocks.at(i) = reinterpret_cast<uint8_t*>(
                    file.m_rawData.data() + ramToFilePos(0x80065950 + (120 * i)));
                shopBuyPrices.at(i) = &Utilities::as<uint16_t>(file.m_rawData,
                                                               ramToFilePos(0x80065c20 + (240 * i)));

                uint16_t* sellTable = nullptr;

                if (i == 5)
                    sellTable = &Utilities::as<uint16_t>(file.m_rawData,
        ...
        case KFMTCore::VersionedGame::KF2U:
            uiStrings1 = &Utilities::as<KF2::PosString>(file.m_rawData,
                                                                 ramToFilePos(0x80064cf0));
            uiStrings2 = &Utilities::as<KF2::String>(file.m_rawData,
                                                              ramToFilePos(0x80065674));
            itemStrings = &Utilities::as<KF2::String>(file.m_rawData,
                                                               ramToFilePos(0x80065b24));
            magicStrings = &Utilities::as<KF2::String>(file.m_rawData,
                                                                ramToFilePos(0x80066664));
            for (size_t i = 0; i < 6; i++)
            {
                shopStocks.at(i) = reinterpret_cast<uint8_t*>(
                    file.m_rawData.data() + ramToFilePos(0x80066844 + (120 * i)));
                shopBuyPrices.at(i) = &Utilities::as<uint16_t>(file.m_rawData,
